...
const imageStorage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    cb(null, `profile_${req.user._id}_${Date.now()}${path.extname(file.originalname).toLowerCase()}`);
  }
});
...
router.post('/avatar', protect, uploadProfileImage.single('avatar'), async (req, res) => {
  try {
    ...
    res.json({ success: true, avatarUrl, message: 'Profile image updated!' });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.delete('/avatar', protect, async (req, res) => {
  try {
    ...
    if (user?.profileImage) {
      const imagePath = path.resolve(__dirname, '..', user.profileImage.replace(/^\/uploads[\\/]/, 'uploads/'));
